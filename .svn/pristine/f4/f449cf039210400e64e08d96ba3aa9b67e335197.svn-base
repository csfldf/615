    ...
    var selector=avalon.define('selector',function(vm){
        ...
        vm.initData = function (success, args) {
            if(types[vm.selectType].data.length>0){
                ...
                var useFunction = !(vm.queryStringFunction == avalon.noop
                                        || vm.queryStringFunction == null
                                        || args == null)
                var address = (!useFunction) ? vm.queryString : vm.queryStringFunction(vm.queryString, args)
                avalon.getJSON(
                    address,
                    function (data) {
                        data = eval(data)
